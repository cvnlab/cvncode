function [mappedvals,Lookup,rgbimg,options] = cmfcvnlookupimages2(subject, vals, hemi, view_az_el_tilt, Lookup, varargin)
% [mappedvals,Lookup,rgbimg] = cmfcvnlookupimages2(subject, vals, hemi, view_az_el_tilt, Lookup, 'param1','value1',...)
%
% Inputs:
...
%                   cmfcvnlookupimages2 (or [] for first call).  Can speed up
...
%               be passed to subsequent cmfcvnlookupimages2 calls
%
%   
% Outputs:
...
% [img,L,rgbimg]=cmfcvnlookupimages2('C0041',M.ecc,'lh',[10 -40 0]);
% 
% % Eccentricity map around occipital pole, thresholded for R2>.15
% [img,L,rgbimg]=cmfcvnlookupimages2('C0041',M.ecc,'lh',[10 -40 0],L,'overlayalpha',M.R2>.15);
% 
% % Polar angle map around occipital pole, explicit colormap limits [0 360]
% [img,L,rgbimg]=cmfcvnlookupimages2('C0041',M.ang,'lh',[10 -40 0],L,'clim',[0 360]);
% 
% % R2 map around occipital pole, explicit colormap limits [0 1], save RGB image to png
% cmfcvnlookupimages2('C0041',M.R2,'lh',[10 -40 0],L,'clim',[0 1],'filename','test_l1_r2.png',);
% 
...
% [img,L,rgbimg]=cmfcvnlookupimages2('C0041',M.ang,'lh',[10 -40 0],L,'roiname','V1');
%
% % draw 2 roi borders.  first one is black, second is white
% [img,L,rgbimg]=cmfcvnlookupimages2('C0041',M.ang,'lh',[10 -40 0],L,'roimask',{roiV1,roiV2},...
%   'roicolor',{[0 0 0],[1 1 1]});
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Quick lookup of mapped values only (No RGB, etc...):
%
% [~,L,~]=cmfcvnlookupimages2('C0041',M.ecc,'lh',[10 -40 0]);
...
% L=[];
% [img1,L,rgbimg1]=cmfcvnlookupimages2('C0045',valstruct,{'lh','rh'},{[10 -40 0],[-10 -40 0]},[]);
% %layer2
% valstruct.data=squeeze(V.data(:,2,:));
% [img2,L,rgbimg2]=cmfcvnlookupimages2('C0045',valstruct,{'lh','rh'},[],L);
%
...
% [img1,L,rgbimg1]=cmfcvnlookupimages2('C0045',valstruct,{'lh','rh'},{[10 -40 0],[-10 -40 0]},[],'mosaic',[3 2]);
...
if(regexpmatch(options.surfsuffix,'^fsaverage'))
    ...
    %Then do cmfcvnlookupimages2 with subject=fsaverage
    ...
    [mappedvals,Lookup,rgbimg,options] = cmfcvnlookupimages2('fsaverage', vals_fsavg, hemi, view_az_el_tilt, Lookup, optargs{:});
    return;
end
...
if(isstruct(vals) && isfield(vals,'numlh'))
    ...
    for i=1:numel(hemi)
        ...
        [imghemi{i},lookuphemi{i},rgbimghemi{i},return_options]=cmfcvnlookupimages2(subject, hemivals, h, v, L, optargs{:});
...
%Save background
pathname = '/home/stone-ext4/generic/Dropbox/hcp7tret-cmf/code/cmfdraw/';
if(~exist(strcat(pathname,'cmfindex.mat')))
    index = 1;
    file2save = fullfile(pathname,'cmfindex.mat');
    save(file2save,'index')
end
indexLoader = load(strcat(pathname,'cmfindex.mat'));
index = indexLoader.index;
if (index == 1)
    curvimgs{index} = rgboptions.background;
    file2save = fullfile(pathname,'cmfcurvimgs.mat');
    save(file2save,'curvimgs')
    index = 2;
else if (index == 2)
        load(strcat(pathname,'cmfcurvimgs.mat'))
        curvimgs{index} = rgboptions.background;
        curvimg = horzcat(curvimgs{1},curvimgs{2});
        file2save = fullfile(strcat('/home/stone-ext4/kendrick/HCP7TFIXED/manualdefinition/lineData/',subject,'/'),'cache.mat');
        save(file2save,'curvimg','-append')
        index = 1;
    end
end
file2save = fullfile(pathname,'cmfindex.mat');
