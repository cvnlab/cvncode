...
datadir = '/stone/ext1/fmridata/20151214-ST001-D002/';
...
WE HAVE TO DO THIS HACK. SO DO THIS MANUALLY.
At the dbstop, do the for-loop in order to massage the fieldmap data.

dbstop in preprocessfmri_standard at 131;

for p=1:length(fieldmaps)
  fieldmaps{p} = circshift(fieldmaps{p},[0 7]);
end
for p=1:length(fieldmapbrains)
  fieldmapbrains{p} = circshift(fieldmapbrains{p},[0 7]);
end
...
epifilenames = epifilenames(1:end-1);  % IGNORE THE LAST RUN (LOCALIZER)
...
extratrans = {cvngetlayersinEPI('C0045',datadir,6,'A','pt')};
...
mcmask = {};
mcmask = { [0.46 0.52 0.1] [0.28 0.34 0.74];};
