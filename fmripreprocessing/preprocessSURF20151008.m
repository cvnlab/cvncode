...
% it is okay if you also match things that are not DICOM directories; we'll just ignore them.
epifilenames = matchfiles([datadir '/MR*bold*run*']);
...
% 84 slices, MB2; get a new sampling rate at 2-s (cubic interpolation)
episliceorder = {};
...
motionreference = [1 137];
motionreference = [1 ceil((137*2.2)/2)];  % we changed the TR
...
mcmask = {};
mcmask = { [0.42 0.5 0.3] [0.28 0.32 0.6];};
...
savefile = [datadir '/preprocessSURF/run%02d.nii'];
savefileB = [datadir '/preprocessSURF/valid.nii'];
savefileC = [datadir '/preprocessSURF/mean.nii'];
